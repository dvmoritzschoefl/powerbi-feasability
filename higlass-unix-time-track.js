(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["higlass-unix-time-track"] = factory();
	else
		root["higlass-unix-time-track"] = factory();
})(window, function() {
...
var UnixTimeTrackScale_UnixTimeTrackScale = function UnixTimeTrackScale(HGC) {
  ...
  var UnixTimeTrackClass = function (_HGC$tracks$TiledPixi) {
    _inherits(UnixTimeTrackClass, _HGC$tracks$TiledPixi);

    function UnixTimeTrackClass(scene, trackConfig, dataConfig, handleTilesetInfoReceived, animate) {
      _classCallCheck(this, UnixTimeTrackClass);
      ...
      _this.axisTextFontSize = 12;
      _this.timeScale = _this._xScale;
      _this.context = new PIXI.Text('sample', {
        fontSize: _this.axisTextFontSize + 'px',
    ...
    _createClass(UnixTimeTrackClass, [{
      ...
      value: function updateTimeScale() {
        ...
        var timeScale = utcTime().domain(linearScale.domain().map(function (d) {
          return d * 1000;
      ...
      key: 'createAxisTexts',
      value: function createAxisTexts() {
        var ticks = this.timeScale.ticks();
        var tickFormat = this.timeScale.tickFormat();
        ...
        while (i < ticks.length) {
          ...
          while (this.axisTexts.length <= i) {
            var newText = new PIXI.Text(tick, {
      ...
      key: 'drawContext',
      value: function drawContext(tickStartY, tickEndY) {
        var ticks = this.timeScale.ticks();
        var center = (+this.timeScale.domain()[1] + +this.timeScale.domain()[0]) / 2;
    ...
    return UnixTimeTrackClass;
  }(HGC.tracks.TiledPixiTrack);
